...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalize.data.problem_credits_five.result", []),
    ...
    isFetched: get(
      state,
      "normalize.data.problem_credits_five.isFetched",
      false
    ),
    meta: get(state, "normalize.data.problem_credits_five.result._meta", {}),
...
const mapDispatchToProps = (dispatch) => {
	return {
      ...
      const storeName = "problem_credits_five";
      const entityName = "problem_credit";
      const scheme = { data: [ProblemCreditScheme] };
      dispatch({
        type: ApiActions.GET_ALL.REQUEST,
        payload: {
          url: "collector/index/hundred-eighty-to-all",
          config: {
            params: {
              "per-page": 10,
