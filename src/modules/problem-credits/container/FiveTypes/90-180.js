  ...
  render() {
    ...
    return (
      <>
        ...
        />
        {!isFetched && <Loader />}
        {isFetched && !isEmpty(drawToRender_data) && (
          <Table body={drawToRender_data} />
        )}
        {isFetched && isEmpty(drawToRender_data) && (
          <p className="search-data">Поиск информации</p>
        )}
        {!isEmpty(drawToRender_data) && (
          <Pagination meta={meta} onChange={this.handlePagination} />
        )}
      </>
    );
  }
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalize.data.problem_credits_four.result", []),
    ...
    isFetched: get(
      state,
      "normalize.data.problem_credits_four.isFetched",
      false
    ),
    meta: get(state, "normalize.data.problem_credits_four.result._meta", {}),
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    callToRender: ({
      ...
      const storeName = "problem_credits_four";
      const entityName = "problem_credit";
      const scheme = { data: [ProblemCreditScheme] };
      dispatch({
        type: ApiActions.GET_ALL.REQUEST,
        payload: {
          url: "collector/index/ninety-to-all",
          config: {
            params: {
              "per-page": 10,
