...
const mapStateToProps = (state) => {
	return {
		drawToRender: get(state, "normalize.data.problem_credits_two.result", []),
		...
		isFetched: get(state, 'normalize.data.problem_credits_two.isFetched', false),
		meta: get(state, "normalize.data.problem_credits_two.result._meta", {}),
...
const mapDispatchToProps = (dispatch) => {
	return {
      ...
      const storeName = "problem_credits_two";
      const entityName = "problem_credit";
      const scheme = { data: [ProblemCreditScheme] };
      dispatch({
        type: ApiActions.GET_ALL.REQUEST,
        payload: {
          url: "collector/index/thirty-to-sixty",
          config: {
						params: {
							'per-page': 10,
