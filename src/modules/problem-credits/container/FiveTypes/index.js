...
class ProblemCredits extends Component {
	state = {
		region_code: '',
		filial_code: '',
		loan_status: '',
...
const mapStateToProps = (state) => {
	return {
		drawToRender: get(state, "normalize.data.problem_credits_all.result", []),
		...
		isFetched: get(state, 'normalize.data.problem_credits_all.isFetched', false),
		meta: get(state, "normalize.data.problem_credits_all.result._meta", {}),
...
const mapDispatchToProps = (dispatch) => {
	return {
      ...
      const storeName = "problem_credits_all";
      const entityName = "problem_credit";
      const scheme = { data: [ProblemCreditScheme] };
      dispatch({
        type: ApiActions.GET_ALL.REQUEST,
        payload: {
          url: "collector/index/one-to-thirty",
          config: {
						params: {
							'per-page': 10,
...
export default connect(mapStateToProps, mapDispatchToProps)(ProblemCredits);
