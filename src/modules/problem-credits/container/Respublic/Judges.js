...
class Judges extends Component {
  ...
  pushedView = (param) => {
    ...
    history.push(`/respublic/judges/view/${loan_id}`);
  };
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalize.data.respublic.result", []),
    regions: get(state, "normalize.data.region.result", []),
    filials: get(state, "normalize.data.filial.result", []),
    isFetched: get(state, "normalize.data.respublic.isFetched", false),
    entities: get(state, "normalize.entities", []),
    _meta: get(state, "normalize.data.respublic.result._meta", {})
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    callToRender: ({ region_code, filial_code, page }) => {
      const storeName = "respublic";
      const entityName = "loan";
      const scheme = { data: [LoanScheme] };
      dispatch({
        type: ApiActions.GET_ALL.REQUEST,
        payload: {
          url: "/problem-credit/juridic/republic",
          config: {
            params: {
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(Judges));
