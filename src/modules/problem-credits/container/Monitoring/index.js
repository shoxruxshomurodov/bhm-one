...
class index extends Component {
  ...
  sendToApi = () => {
    ...
    history.push(`/monitoring/${address}`);
  };
  pushedView = (param) => {
    const { loan_id } = param;
    const { history } = this.props;
    history.push(`/monitoring/view/${loan_id}`);
  };
  render() {
    const {
      drawPage,
...
const mapStateToProps = (state) => {
  return {
    drawPage: get(state, "normalize.data.monitoring.result", []),
    regions: get(state, "normalize.data.region.result", []),
    filials: get(state, "normalize.data.filial.result", []),
    isFetched: get(state, "normalize.data.monitoring.isFetched", false),
    _meta: get(state, "normalize.data.monitoring.result._meta", {}),
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    callToRender: ({ region_code, filial_code, loan_code, period, page }) => {
      const storeName = "monitoring";
      const entityName = "loan";
      const scheme = { data: [LoanScheme] };
      dispatch({
        type: ApiActions.GET_ALL.REQUEST,
        payload: {
          url: "/problem-credit/monitoring/index",
          config: {
            params: {
