...
class index extends Component {
  ...
  sendToApi = () => {
    ...
    history.push(`/formed-list/${address}`);
  };
  pushedView = (param) => {
    const { loan_id } = param;
    const { history } = this.props;
    history.push(`/formed-list/view/${loan_id}`);
  };
...
const mapStateToProps = (state) => {
  return {
    drawToRender: get(state, "normalize.data.formed_list.result", []),
    regions: get(state, "normalize.data.region.result", []),
    filials: get(state, "normalize.data.filial.result", []),
    isFetched: get(state, "normalize.data.formed_list.isFetched", false),
    entities: get(state, "normalize.entities", []),
    _meta: get(state, "normalize.data.formed_list.result._meta", {})
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    callToRender: ({ region_code, filial_code, page }) => {
      const storeName = "formed_list";
      const entityName = "loan";
      const scheme = { data: [LoanScheme] };
      dispatch({
        type: ApiActions.GET_ALL.REQUEST,
        payload: {
          url: "/problem-credit/juridic/index",
          config: {
            params: {
