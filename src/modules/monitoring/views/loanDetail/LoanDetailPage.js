...
import LoanDetailScheme from "../../../../schema/LoanDetail";
...
function LoanDetailPage(props) {
  ...
  let data = useSelector((state) =>
    get(state, "normalize.data.loan-detail-list.result", [])
  );
  const isFetchedData = useSelector((state) =>
    get(state, "normalize.data.loan-detail-list.isFetched", true)
  );
  const meta = useSelector((state) =>
    get(state, "normalize.data.loan-detail-list.result._meta", [])
  );
  const entities = useSelector((state) => get(state, "normalize.entities", []));
  data = get(
    Normalizer.Denormalize(data, { data: [LoanDetailScheme] }, entities),
    "data",
    []
  );
  ...
  const getAllData = (params = {}) => {
    const storeName = "loan-detail-list";
    const entityName = "loan-detail";
    const scheme = { data: [LoanDetailScheme] };
    dispatch({
  ...
  return (
    <div className={"d-flex flex fixed-content mt-2 problem-filter-aside"}>
      <FilterAside title={t("Loan Details")} classname="header">
        <div className="hover">
          <div className="sidenav p-2">
            <nav className="nav-active-text-primary">
              <ul className="nav">
                <li
                  className={isEqual(type, "all") && "active"}
                  onClick={() => {
                    setType("all");
                    getAllData({ page: 1, type: "all" });
                  }}
                >
                  <a>
                    <span className="nav-text">{t("Жами")}</span>
                    <span className="nav-badge">
                      <b className="badge badge-sm badge-pill">
                        {get(meta, "counts.all_count")}
                      </b>
                    </span>
                  </a>
                </li>
                <li
                  className={isEqual(type, "day_1_30") && "active"}
                  onClick={() => {
                    setType("day_1_30");
                    getAllData({ page: 1, type: "day_1_30" });
                  }}
                >
                  <a>
                    <span className="nav-text">{t("1-30 кунгача")}</span>
                    <span className="nav-badge">
                      <b className="badge badge-sm badge-pill bg-blue-light">
                        {get(meta, "counts.day_1_30")}
                      </b>
                    </span>
                  </a>
                </li>
                <li
                  className={isEqual(type, "day_31_60") && "active"}
                  onClick={() => {
                    setType("day_31_60");
                    getAllData({ page: 1, type: "day_31_60" });
                  }}
                >
                  <a>
                    <span className="nav-text">{t("31-60 кунгача")}</span>{" "}
                    <span className="nav-badge">
                      <b className="badge badge-sm badge-pill bg-blue">
                        {get(meta, "counts.day_31_60")}
                      </b>
                    </span>
                  </a>
                </li>
                <li
                  className={isEqual(type, "day_61_90") && "active"}
                  onClick={() => {
                    setType("day_61_90");
                    getAllData({ page: 1, type: "day_61_90" });
                  }}
                >
                  <a>
                    <span className="nav-text">{t("61-90 кунгача")}</span>{" "}
                    <span className="nav-badge">
                      <b className="badge badge-sm badge-pill bg-blue-dark">
                        {get(meta, "counts.day_61_90")}
                      </b>
                    </span>
                  </a>
                </li>
                ...
                >
                  <a>
                    <span className="nav-text">{t("91-180 кунгача")}</span>{" "}
                    <span className="nav-badge">
                ...
                >
                  <a>
                    <span className="nav-text">{t("181-360 кунгача")}</span>{" "}
                    <span className="nav-badge">
                ...
                >
                  <a>
                    <span className="nav-text">{t("360 кундан ортик")}</span>{" "}
                    <span className="nav-badge">
                ...
                <li
                  className={isEqual(type, "exception") && "active"}
                  onClick={() => {
                    setType("exception");
                    getAllData({ page: 1, type: "exception" });
                  }}
                >
                  <a>
                    <span className="nav-text">{t("Exception")}</span>
                    <span className="nav-badge">
                      <b className="badge badge-sm badge-pill text-dark">
                        {get(meta, "counts.exceptions_count")}
                      </b>
                    </span>
                  </a>
                </li>
            ...
            <div className={"scroll-y mx-3 problem-table"}>
              <ReactTableFixedColumns
                ...
                getTrProps={(state, rowInfo, column, instance) => {
                  return {
                    onClick: () => {
                      setActive(get(rowInfo, "original.importData.LOAN_ID"));
                    },
                    onDoubleClick: (e, t) => {
                      history.push(
                    ...
                    className: isEqual(
                      get(rowInfo, "original.importData.LOAN_ID"),
                      active
                    )
                      ? "active text-hover"
                      : "text-hover",
...
export default withTranslation("bhm_one")(LoanDetailPage);
