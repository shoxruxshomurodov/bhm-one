...
function StagesUpdatePage(props) {
    const {id} = useParams();
    const {t} = props;
    ...
    const update = (data) => {
        setIsFetched(true)
        const storeName = "credit-monitoring-stages-update";
    ...
    const cancelLink = () => {
        history.push(`/credit-monitoring/stages/view/${id}`)
    }
    ...
    return (
        <>
            <PageHeader name={t("Stages")} desc={t("Stage update")}/>
            <div className={"page-content page-container"}>
                <div className={"card"}>
                    <div className="card-header"><strong>{t("Update stage")}</strong></div>
                    <div className={"card-body"}>
                        <Formik
                            initialValues={{
                                title: get(stage, "title"),
                                comment: get(stage, "comment"),
                                start_day: get(stage, "start_day"),
                                expire_day: get(stage, "expire_day")
                            }}
                            // validate={values => {
                            //     const errors = {};
                            //     if (!values.title) {
                                ...
                                <form onSubmit={handleSubmit}>
                                    <div className="form-group row"><label
                                        ...
                                        <div className="col-sm-9"><input
                                            ...
                                            <small
                                                className={"text-danger"}>{errors.title && touched.title && errors.title}</small>
                                        </div>
                                    </div>
                                    <div className="form-group row"><label
                                    ...
                                    <div className="form-group row"><label
                                        ...
                                        <div className="col-sm-9"><input
                                            ...
                                            className="form-control"/>
                                            <small
                                                className={"text-danger"}>{errors.start_day && touched.start_day && errors.start_day}</small>
                                        </div>
                                    </div>
                                    <div className="form-group row"><label
                                        ...
                                        <div className="col-sm-9"><input
                                            ...
                                            className="form-control"/>
                                            <small
                                                className={"text-danger"}>{errors.expire_day && touched.expire_day && errors.expire_day}</small>
                                        </div>
                                    </div>
                                    <button className={"btn  btn-primary mr-2"} disabled={isSubmitting}
                                            type="submit">{t("Save")}</button>
                                    <button onClick={cancelLink} className={"btn btn-default"}
...
export default withTranslation("bhm_one")(StagesUpdatePage);
