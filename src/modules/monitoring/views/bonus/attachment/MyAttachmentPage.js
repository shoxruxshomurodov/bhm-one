...
function MyAttachmentPage(props) {
    const {t} = props;
    const [active, setActive] = useState(false)
    const dispatch = useDispatch();
    const isFetched = useSelector((state) =>
        get(state, "normalize.data.credit-monitoring-my-loans.isFetched", false)
    );
    let loansList = useSelector((state) =>
        get(state, "normalize.data.credit-monitoring-my-loans.result", [])
    );
    const meta = useSelector((state) =>
        get(state, "normalize.data.credit-monitoring-my-loans.result._meta", [])
    );
    ...
    const getAllLoans = (params) => {
        const storeName = "credit-monitoring-my-loans";
        ...
        dispatch({
            ...
            payload: {
                url: "/bonus/bonus-loan/my-assigned",
                config: {
                    params: {
                        "per-page": 10,
                        include: "region",
        ...
        <div className={"d-flex flex fixed-content mt-2"}>
            <div className={"d-flex flex"} id={"content-body"}>
                <div className={"d-flex flex-column flex"} id={"user-list"}>
                    <Hat
                        name="Муаммоли деб топилган кредитлар рўйҳати"
                        desc="Сизга бириктирилган кредитлар"
                    />
                        <TemplateTable
                            className="scroll-y mb-0 px-2 mt-2"
                            head={[
                                '№',
                                'ID',
                                "Регион",
...
export default withTranslation("bhm_one")(MyAttachmentPage);