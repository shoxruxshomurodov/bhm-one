...
export function UpdateContainer({id, getAll, setOpenUpdateForm , t}) {
    const prevId = usePrevious(id);
    const dispatch = useDispatch();
    const entities = useSelector((state) => get(state, "normalize.entities"));
    const isFetchedOne = useSelector((state) =>
        get(state, "normalize.data.workflow-process-product-update-one.isFetched",false)
    );
    const resultOne = useSelector((state) =>
        get(state, "normalize.data.workflow-process-product-update-one.result")
    );
    ...
    const dataOne = Normalizer.Denormalize(resultOne, workflowProcessProduct, entities);
    const getData = () => {
        dispatch({
            ...
            payload: {
                ...
                scheme: workflowProcessProduct,
                storeName: "workflow-process-product-update-one",
    ...
    const update = (attributes, formMethods) => {
        ...
        const storeName = "workflow--product-update";
    ...
    return (
        <UpdateForm attributes={dataOne} state_list={resultData} update={update} t={t} />
    );
}
