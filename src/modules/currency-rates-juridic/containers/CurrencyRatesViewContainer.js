...
class CurrencyRatesViewContainer extends Component {
    ...
    getCurrency = () => {
        ...
        getOneCurrency({id: currency_id, api_url: `/course/passport-course/${currency_id}`});
    }
    signDoc = async () => {
        const {currency} = this.props;
        const {certinfo,hash} = this.state;
        const params = {certInfo:certinfo, id: get(currency, 'id', null), signedMsg: isEmpty(hash) ? "e0d123e5f316bef78bfdf5a008837577":hash};
    ...
    render() {
        ...
        const exchangePointsList = get(currency, 'onlineConversions', []);
        const employee = get(currency, 'employee', {});
        const {id, order_date, order_number, qrcode, status,created_at} = currency;
        return (
            <div className={"page-content padding"}>
                <div className="row">
                    <div className="col-md-12">
                        <ToastContainer/>
                        <QRCode style={{display:'none'}}  value={`http://new.xb.uz:8081/uz/course-passport/${get(qrcode, 'url', '')}`} />
                    </div>
                </div>
                        ...
                        <div className="row">
                            <div className="col-md-12">
                                <PdfViewForm qrcode={qrcode} status={status} exchangePointsList={exchangePointsList}
                                               id={id} order_date={order_date}
                                             order_number={order_number} employee={employee} created_at={created_at} sign={this.signDoc}/>
                            </div>
                        </div>
        ...
        getOneCurrency: ({id, api_url}) => {
            ...
            dispatch({
                ...
                payload: {
                    ...
                    config: {
                        params: {
                            include: 'exchangePoints,onlineConversions,employee'
                        },
                    },
                    scheme,
                    storeName,
