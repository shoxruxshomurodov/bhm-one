...
class CurrencyRatesListTodayContainer extends Component {
                                            ...
                                            <BaseTable head={['ID','БУЮРТМА САНАСИ','БУЮРТМА РАҚАМИ','ИМЗО','ХОДИМ','ЯРАТИЛГАН САНА']} className={"mt-3"}>
                                                {
                                                    currencyList && currencyList.map(({id,order_date,order_number,status,employee={},created_at}, index) => <tr
                                                        key={id}
                                                        style={{verticalAlign: 'middle'}} onDoubleClick={() => history.push(`/currency-juridic/view/${id}`)}>
                                                        <td>{id}</td>
                                                        <td>{order_date}</td>
...
const mapStateToProps = (state) => {
    return{
        ...
        currencyList:get(state,'normalize.data.currency-today-list.result.data',[]),
        meta:get(state,'normalize.data.currency-today-list.result._meta',{}),
        isFetched:get(state,'normalize.data.currency-today-list.isFetched',false)
    }
}
...
const mapDispatchToProps = (dispatch) => {
    return{
        getAllCurrency: ({page = 1}) => {
            const storeName = 'currency-today-list';
            const entityName = 'currency';
            const scheme = {data: [CurrencyListScheme]};
            dispatch({
                type: ApiActions.GET_ALL.REQUEST,
                payload: {
                    url: '/course/passport-course/today',
                    config: {
                        params: {
                            include:'employee',
                            page
...
export default connect(mapStateToProps,mapDispatchToProps)(withRouter(CurrencyRatesListTodayContainer));