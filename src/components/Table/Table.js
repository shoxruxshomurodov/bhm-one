					...
					<tr>
						{head &&
							...
							head.map((th, index) => {
								return (
									<th
										className={haveCheckbox ? `credit_th_${index}` : `credit_th_${index + 1}`}
										key={index}
									>
										{th}
									</th>
						...
						body.map((td, index) => {
							return (
								...
								>
									...
									{haveCheckbox && (
										<td onClick={(e) => haveCheckbox && checkedIdsCredits(e.target.dataset.id)}>
											<input type="checkbox" data-id={get(td, 'loan_id')} />
										</td>
									)}
									...
									{isEqual(get(td, 'category'), 'SEND_TO_JURIDIC') && (
										<td className="text-warning">Юристга юборилган</td>
									)}
								</tr>
							);
						})}
